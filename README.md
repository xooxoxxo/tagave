...
Liner safely corrects tags in your music files using a canonical schema derived from MusicBrainz and Discogs metadata. The tag-correction workflow is **journaled and fully revertible**, with built-in safeguards against accidental data loss.
...
The canonical schema ensures consistent field names across MP3, FLAC, M4A/ALAC, OGG, Opus, WAV, and AIFF — each with different native tag names and conventions. See Appendix A in the specification for the complete mapping.
...
2. **Apply:** Write changes through the tag writer sidecar (mutagen for Python; taglib via WASM in future). An audio-hash is computed before and after the write. If the audio-hash doesn't match, the file is marked failed and the job pauses (a P0 defect, never a retry). Hashes are stored in the audit log for every write.
...
Liner enforces safe write limits for each container format:

- **APE / WavPack:** read-only in v1. Liner reads existing APE tags but does not write them; these container formats require additional validation and testing before write support is added.
- **WV (WavPack):** read-only in v1, same constraint as APE. No writes to WV files until corpus round-trip passes.
- **Cue-image albums:** write only album-level fields to the image file. The "cue-image album-level" rule forbids writing track titles, track numbers, or splitting the audio file when the album is sourced from a CUE sheet. (See the **XO-314 owner rule** in build documentation.)
